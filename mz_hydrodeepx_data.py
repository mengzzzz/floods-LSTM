...
#     dataset["tmean"] = dataset['tmean(C)']
#     dataset["prcp"] = dataset['prcp(mm/day)']
#     dataset["GW"] = dataset['GW(m)']
#     dataset["ST"] = dataset['flow(mm)']
#     dataset = dataset.drop(["tmean(C)", "prcp(mm/day)", "flow(mm)", "dayl(day)", "srad(W/m2)", "vp(Pa)", "GW(m)"], axis=1)
    dataset = dataset.replace(-99, np.NaN)
    dataset = dataset.set_index("date")
    dataset = dataset.dropna()
    dataset = dataset.loc[pd.date_range(start='1/1/1968', end='20/12/1969')]
    ...
    for date_i in tqdm(dataset.index, desc=f'Prep aring data with wrap length = {wrap_length}'):
        try:
            data_x = dataset.loc[pd.date_range(end=date_i,
                                               ...
                                               freq="d")[:-1], ["R","NAO"], ].to_numpy(dtype='float16')
...
def split_train_test(dataset, data_x_dict, data_y_dict, frac=0.7, random_state=100, scale=True):
    ...
    if scale is False:
        ...
        scale_params["train_x_mean"] = (dataset.loc[train_dates, ["R","NAO"]].mean().values)
        scale_params["train_x_std"]  = (dataset.loc[train_dates, ["R","NAO"]].std().values)
